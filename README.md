...
| -------|---------|---------------|
| `GH_USERNAME` | The github username | "sivaselvi106" |
...
| `KEYBASE_USERNAME` | Keybase username | "sivaselvi" |
...
1. **_Only in this README file and `files/.zprofile` files (and nowhere else):_** Find and replace the strings that reference my usernames to your equivalent ones (for eg, you can search for `sivaselvi106` and `sivaselvi` and replace them with your values).
...
```zsh
export GH_USERNAME="sivaselvi106"; export DOTFILES_DIR="${HOME}/.bin-oss"; curl -fsSL "https://raw.githubusercontent.com/${GH_USERNAME}/dotfiles/master/scripts/fresh-install-of-osx.sh" | zsh
```
